...
[**API 설계 기획서 및 Postman 요청-응답 확인**](#api-docs)

[**개발 과정 기록**](#devlog)
1. [기존 API 설계서 재확인 및 수정](#api-review)
2. [역할, 책임 기준 컨트롤러 분리](#controller-separation)
3. [사용자 로그인 여부 검사 방식 변경](#auth-strategy)
4. [토큰 프로바이더 인터페이스 구현](#token-provider)
5. [SecurityConfig 내부 불필요 멤버 제거](#securityconfig-refactor)
6. [토큰 검사 루프 반복](#token-loop)
7. [Spring Security, LogoutFilter](#logout-filter)
...
## <a id="devlog"></a>1. 개발 과정 기록

### <a id="api-review"></a>1-1) 기존 API 설계서 재확인 및 수정
...
### <a id="controller-separation"></a>1-2) 역할, 책임 기준 컨트롤러 분리
...
### <a id="auth-strategy"></a>1-3) 사용자 로그인 여부 검사 방식 변경
...
### <a id="token-provider"></a>1-4) 토큰 프로바이더 인터페이스 구현
...
### <a id="securityconfig-refactor"></a>1-5) SecurityConfig 내부 불필요 멤버 제거
...
### <a id="token-loop"></a>1-6) 토큰 검사 루프 반복
...
### <a id="logout-filter"></a>1-7) Spring Security, LogoutFilter
